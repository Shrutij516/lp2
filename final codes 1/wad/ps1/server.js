...
//Connect to mongo
// 'mongodb+srv://shrutijagdale16:user123@cluster0.1ygqnyd.mongodb.net/?retryWrites=true&w=majority'
mongoose.connect('mongodb://127.0.0.1:27017/1', { useNewUrlParser: true, useUnifiedTopology: true })
...
app.put('/update', async (req, res) => {
    ...
    try {
      ...
      const update = { $inc: { WAD_Marks: wad, CC_Marks: cc, DSBDA_Marks: dsbda, CNS_Marks: cns, AI_marks: ai } };
...
app.get('/allAbove25', async (req, res) => {
  try {
    const students = await Student.find({WAD_Marks: { $gt: 25 }, CC_Marks: { $gt: 25 }, DSBDA_Marks: { $gt: 25 }, CNS_Marks: { $gt: 25 }});
    res.send(students)
  } catch (err) {
    res.send(err);
  }
...
app.get('/lowMathScience', async (req, res) => {
  try {
    const students = await Student.find({WAD_Marks: { $lt: 40 }, CC_Marks: { $lt: 40 }});
    res.send(students)
  } catch (err) {
    res.send(err);
  }
